...
|---|---|---|---|---|---|---|
| `RoundNotClosable` | `RETRYABLE` | State unchanged on failure; retry allowed after `now >= close_ts`. | N/A | `programs/openjack/src/errors.rs:10` | `programs/openjack/src/instructions/round.rs` (`validate_begin_freeze_state`) | `programs/openjack/src/instructions/round.rs` (`begin_freeze_validation_rejects_before_close`) |
| `FreezeStateInvalid` | `RETRYABLE` | State unchanged on failure in freeze apply path; retry allowed once round reaches a valid freeze state. | N/A | `programs/openjack/src/errors.rs:70` | `programs/openjack/src/instructions/round.rs` (`validate_begin_freeze_state`, `apply_prototype_freeze`) | `programs/openjack/src/instructions/round.rs` (`begin_freeze_validation_rejects_wrong_state`, `apply_prototype_freeze_rejects_invalid_state`) |
| `FreezeCommitmentMismatch` | `HARD_STOP` | N/A | Treat as integrity violation. Do not bypass with timeout. Investigate canonical-input divergence/state corruption, patch root cause, and only then resume freeze progression. | `programs/openjack/src/errors.rs:72` | `programs/openjack/src/instructions/round.rs:291` (`apply_prototype_freeze`) | `programs/openjack/src/instructions/round.rs:505` (`apply_prototype_freeze_rejects_commitment_mismatch`) |
| `FreezeSourceInvalid` | `HARD_STOP` | N/A | Treat as state/config defect. No timeout bypass. Requires code/data correction before progress. | `programs/openjack/src/errors.rs:74` | `programs/openjack/src/instructions/round.rs` (`validate_freeze_source_bounds`) | `programs/openjack/src/instructions/round.rs` (`freeze_source_bounds_reject_inverted_range`) |
| `MathOverflow` (freeze path) | `HARD_STOP` | N/A | Treat as implementation defect. No timeout bypass. Patch arithmetic path and re-validate before resume. | `programs/openjack/src/errors.rs:58` | `programs/openjack/src/instructions/round.rs` (`apply_prototype_freeze`) | `programs/openjack/src/instructions/round.rs` (`apply_prototype_freeze_rejects_attempt_counter_overflow`) |
