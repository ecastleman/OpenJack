...
if [[ -z "${OPENJACK_PROGRAM_ID:-}" ]]; then
  echo "OPENJACK_PROGRAM_ID is required for prod-like launch."
  echo "Example:"
  echo "  export OPENJACK_PROGRAM_ID=2AWsuApMg1gr4e9Ybc5Uji5cJnjYDjYaqQzjn6s6draX"
  exit 1
fi
...
export OPENJACK_CLOSE_IN_SECS="${OPENJACK_CLOSE_IN_SECS:-900}"
...
echo "[launch-prod-like] Creating round..."
ROUND_OUTPUT="$(npm run round:create-open:prod-like)"
echo "$ROUND_OUTPUT"

ROUND_ID="$(echo "$ROUND_OUTPUT" | awk -F= '/^round_id=/{print $2}' | tail -n 1 | tr -d '[:space:]')"
if [[ -z "$ROUND_ID" ]]; then
  echo "[launch-prod-like] Could not parse round_id from output."
...
echo "[launch-prod-like] Starting vertical stack for round $ROUND_ID ..."
