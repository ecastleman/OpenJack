...
const ORACLE_MAX_AGE_SECS = Number(process.env.OPENJACK_ORACLE_MAX_AGE_SECS || 600);
...
async function main() {
  ...
  const cfg = await program.account.lotteryConfig.fetch(config);
  const configuredOracle = cfg.solUsdOracle || cfg.sol_usd_oracle;
  ...
  console.log(`oracle_max_age_secs=${ORACLE_MAX_AGE_SECS}`);
}
